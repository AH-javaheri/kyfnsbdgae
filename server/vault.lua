-- ─────────────────────────────────────────────────────────────
-- Vault / armory: a shared apex_inventory stash per gang, namespaced by id.
...
-- Register/refresh the stash so apex_inventory knows its current size.
...
function Vault.Refresh(gangId)
    ...
    exports.apex_inventory:RegisterStash(
...
function Vault.Open(src)
    ...
    exports.apex_inventory:forceOpenInventory(src, 'stash', Vault.StashId(gang.id))
...
function Vault.LootStacks(gangId, maxStacks)
    ...
    local items = exports.apex_inventory:GetInventoryItems(stash)
    ...
    for i = 1, math.min(maxStacks, #pool) do
        local item = pool[i]
        local removed = exports.apex_inventory:RemoveItem(stash, item.name, item.count, item.metadata)
        if removed then
