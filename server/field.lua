...
local function needsItem(src, item)
    ...
    local count = exports.apex_inventory:GetItemCount(src, item)
    return (count or 0) >= 1
end
...
function Field.SearchRob(src, targetSrc)
    ...
    if not Config.Radial.rob.cashOnly then
        local items = exports.apex_inventory:GetInventoryItems(t)
        local pool = {}
        ...
        for i = 1, math.min(Config.Radial.rob.maxItemStacks, #pool) do
            local item = pool[i]
            if exports.apex_inventory:RemoveItem(t, item.name, item.count, item.metadata) then
                exports.apex_inventory:AddItem(src, item.name, item.count, item.metadata)
...
function Field.SlashTyre(src, netId, tyreIndex)
    ...
    if cfg.item and cfg.item ~= '' and cfg.consume then
        exports.apex_inventory:RemoveItem(src, cfg.item, 1)
    end
