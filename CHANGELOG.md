...
- **Every apex_target option in the resource could silently not exist.** Whether apex_target was running got asked once while the files were still being parsed and cached — if this resource started first, the answer was "no" forever, and no prop, ped, dealer or van ever offered an option. It is asked when needed now
...
- **A failed progress bar locked the inventory out.** ox_lib raises `invBusy` for the duration of a bar and lowers it at the end; a bar that throws never reaches the second half, and apex_inventory then refuses to open with "cannot open inventory (is busy)" until the player reconnects. All nine bars in the resource go through one guard now that checks the animation first and always puts the flag back
