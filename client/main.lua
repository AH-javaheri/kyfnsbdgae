...
-- apex_target may still be starting when this file is parsed, so asking
...
function XSTarget.Ready()
    return GetResourceState('apex_target') == 'started'
end
...
-- and the flag stays up forever — apex_inventory then refuses to open with
...
-- apex_inventory client-side usable item: item.client.export = 'XS-CriminalTablet.useDevice'
...
local fallbackPrompt = nil -- { coords, label, action } when apex_target isn't handling the current step
...
local function clearPickupZone()
    if pickupZoneId then
        pcall(function() exports.apex_target:removeZone(pickupZoneId) end)
        pickupZoneId = nil
    end
end
...
local function clearHeistZone()
    if heistZoneId then
        pcall(function() exports.apex_target:removeZone(heistZoneId) end)
        heistZoneId = nil
    end
end
...
local function clearCourierZone()
    if courierZoneId then
        pcall(function() exports.apex_target:removeZone(courierZoneId) end)
        courierZoneId = nil
    end
end
...
-- Single proximity+[E] loop backing whichever interaction apex_target isn't
-- covering right now (no apex_target installed, or the zone/target call failed).
CreateThread(function()
    local shown = false
    while true do
        Wait(300)
...
local function setupPickupTarget(coords)
    ...
    if hasTarget() then
        local ok, id = pcall(function()
            return exports.apex_target:addSphereZone({
...
local function setupVanBackTarget(van, label, onArrive)
    ...
    if hasTarget() then
        local ok = pcall(function()
            exports.apex_target:addLocalEntity(van, {
    ...
    -- No apex_target — poll the van's current (moving) rear offset instead of
...
local function spawnQuartermaster(coords, model, isLeader, onTalk)
    ...
    if hasTarget() then
        local ok = pcall(function()
            exports.apex_target:addLocalEntity(quartermasterPed, {
...
local function setupDropoffTarget(coords, model, isLeader, label, action)
    ...
    if hasTarget() then
        local ok = pcall(function()
            exports.apex_target:addLocalEntity(dropoffPed, {
