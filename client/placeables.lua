...
local function despawnAll()
    for _, handle in pairs(spawned) do
        if DoesEntityExist(handle) then
            if hasTarget() then pcall(function() exports.apex_target:removeLocalEntity(handle) end) end
            DeleteEntity(handle)
        end
    end
    spawned = {}
...
local function spawnOne(p)
    ...
    if hasTarget() then
        exports.apex_target:addLocalEntity(handle, {
            {
...
-- ── proximity prompts (fallback when apex_target isn't installed) ──
-- The thread always runs and asks per tick, because whether apex_target is
